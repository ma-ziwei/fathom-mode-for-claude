...
The Score is intentionally asymptotic to 100% but never reaches it — perfect understanding is a lie. Treat 85%+ as "deep enough to compile if the user wants to."
...
Enough understanding for a useful structured plan. Signal readiness:

- In your response, mention that the session is in compile-ready territory and the user could `/fathom-compile` if ready.
- Don't push them to compile — let them choose. Some users want to go deeper before committing.
...
- **Score is not a stopping rule.** Compilation is always user-triggered via `/fathom-compile`. Never auto-compile.
