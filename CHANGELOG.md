...
- `init_session.py` no longer returns a `score_block_str` field when called without `--nodes`, and `SKILL.md` now explicitly requires Claude to call `update_graph.py` on the first user message (as turn 1). Previously, `init_session.py` emitted its own `score_block_str` which Claude — following the instruction to "place score_block_str verbatim" — would display and then skip the real scoring step, resulting in the first turn always showing a placeholder rather than a computed score. An intermediate attempt to fix this by restoring a 35–55% stub baseline made the laziness loop more believable rather than breaking it; this fix removes the shortcut entirely. The real score now always comes from `update_graph.py`. Claude can either follow the new two-step turn-1 instructions, or — as a safety net — call `update_graph.py` directly (the earlier auto-bootstrap fix still creates the session if none exists).
