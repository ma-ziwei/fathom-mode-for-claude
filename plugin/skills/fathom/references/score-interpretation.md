...
The Score is intentionally asymptotic to 100% but never reaches it — perfect understanding is a lie. The plan-readiness threshold is **50%**: at or above that mark, `update_graph.py` emits a non-empty `plan_hint_str` and the user is invited to `/fathom:plan`. Below 50%, the hint stays empty.
...
Enough understanding for a useful plan. `update_graph.py` now emits the `plan_hint_str` — append it verbatim per the in-session reminder. The user can plan whenever they want; do not push.
...
- **Score is not a stopping rule.** Planning is always user-triggered (`/fathom:plan` or saying "plan"). Never auto-plan even after the hint appears.
